...
class RoutingPlugin:
    ...
    async def corridor_score(self, geometry: dict[str, Any], depart_time: str, arrive_by: str) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=20.0) as client:
            resp = await client.post(f'{COMPOUND_API_URL}/routes/score', json={'geometry': geometry, 'depart_time': depart_time, 'arrive_by': arrive_by, 'run_id': 'latest', 'timestep': 0})
            resp.raise_for_status()
            return resp.json()

    async def _call(self, path: str, payload: dict[str, Any]) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=20.0) as client:
            resp = await client.post(f'{ROUTING_API_URL}{path}', json={'payload': payload})
            resp.raise_for_status()
            return resp.json()
...
async def agent_mitigation(request: MitigationIn) -> dict[str, Any]:
    ...
    depart = request.shipment['depart_time']
    arrive = request.shipment['arrive_by']
