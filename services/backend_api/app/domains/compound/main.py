...
from app.domains.compound.config import Settings, load_settings
from app.domains.compound.hazards.generator import HazardGenerator
from app.domains.compound.logging_utils import configure_logging, next_request_id, request_id_var
from app.domains.compound.storage import Storage
from app.domains.compound.weather.google_weather_client import GoogleWeatherClient
from app.providers.valhalla import route as valhalla_route

configure_logging('compound_api')
logger = logging.getLogger(__name__)
VALHALLA_URL = getenv('VALHALLA_URL', 'http://valhalla:8002').rstrip('/')
...
async def route_options(body: RouteOptionsIn) -> dict[str, Any]:
    ...
    try:
        ...
        options = valhalla.get('routes', [])
    except httpx.HTTPStatusError as exc:
        logger.exception('valhalla returned status=%s body=%s', exc.response.status_code, exc.response.text)
        raise HTTPException(status_code=502, detail='valhalla returned error') from exc
    except httpx.RequestError as exc:
        logger.exception('valhalla unavailable: %s', exc)
        raise HTTPException(status_code=503, detail='valhalla unavailable') from exc
    if not options:
        raise HTTPException(status_code=502, detail='valhalla returned no routes')
